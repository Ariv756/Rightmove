...
/**
 * Leaving this here as im not sure if its required for the next part.
 */
...
const PropertyListing = () => {
    ...
    const fetchData = async () => {
        const call = await fetch('http://localhost:3000/api/properties');
    ...
    return (
        <div className="PropertyListing">
            {
                data.map((property, index) => <PropertyCard key={index} {...property}/>)
            }
        </div>
    )
};
